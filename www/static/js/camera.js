	...
	function nextSlide(navSlide){
		...
		if(!$('.imgLoaded',slide).length){
			...
			if (!$(imgLoaded).get(0).complete || wT == '0' || hT == '0' || typeof wT === 'undefined' || wT === false || typeof hT === 'undefined' || hT === false) {
				$('.camera_loader',wrap).fadeIn(fadeSpeed);
				imgLoaded.onload = function() {
					wT = imgLoaded.naturalWidth;
					hT = imgLoaded.naturalHeight;
					$(imgLoaded)
						.attr('data-alignment',allAlign[slideI]).attr('data-portrait',allPor[slideI])
						.attr('width',wT)
						.attr('height',hT);
			...
			if( allImg.length > (slideI+1) && !$('.imgLoaded',slideNext).length ){
				...
				imgLoaded2.onload = function() {
					...
					$(imgLoaded2)
						.attr('data-alignment',allAlign[slideI+1]).attr('data-portrait',allPor[slideI+1])
						.attr('width',wT)
						.attr('height',hT);
					resizeImage();
				};
			...
			if($('.camera_loader',wrap).is(':visible')){
				$('.camera_loader',wrap).fadeOut(fadeSpeed);
			} else {
				$('.camera_loader',wrap).css({'visibility':'hidden'});
				$('.camera_loader',wrap).fadeOut(fadeSpeed,function(){
